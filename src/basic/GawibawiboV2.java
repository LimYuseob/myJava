...
public class GawibawiboV2 {
	...
	public static void main(String[] args) {
		 ...
		 * 3.1 이 클래스는 play() 에서 객체를 만들어 호출되어야합니다.
		 * 3.2 play() 는 사용자 ID 를 입력 받아서 필드로 저장을 해야 합니다.
		 * 3.3 필요에 따라서 사용자의 ID 가 뭔지를 리턴 받을 수 있는 getter() 를 구현하세요
		 * 3.4 이 클래스 <-- 여기까지가 클래스1
		 * 3.5 여기서부터 클래스 2 -->내부에는 사용자 이름을 입력 받고 난후 실제 게임을 진행 하는
		 * 클래스의 객체를 생성합니다. 이때 객체 생성 후 특정 메서드를 호출 하지 않아도
		 * 내부적으로 호출하여사용자에게 게임 하겠는지 물어봅니다.
		 ...
		 * 
		 * 2를 입력하면 전적보기가 출력 되어야 하는데, 이 내용을 출력한 클래스 3을 정의해서
		 * 이 객체를 이용, 전적을 보여주세요.(당연히 메서드와 생성자 등을 필요에 따라 적절히 연동해야 합니다.
		 * 전적은 몇승 : 몇무 : 몇패 이렇게 출력됩니다.)
		 * 
