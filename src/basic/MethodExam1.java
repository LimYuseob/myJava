...
public class MethodExam1 {
	 ...
	 *  
	 *  어떤 로직이 반복적으로 필요할 경우 해당 로직을 분리해서 메서서드로 정의 해놓으면
	 *  그 기능이 필요할 때마다 사용할 수 있기에 재활용 적인 면에서도 좋습니다.ㅏ
	 *  
	 ...
	 *  메서드가 정의될 때 어떤 파라미터 값이 필요가 없을때
	 *  그냥 괄호만, 만약 필요시엔 해당 값의 타입과, 그 값을 local 에서 사용될 변수명을
	 *  넣어줍니다. 이렇게 선언된 변수명은 해다 메서드의 지역 변수로 활용 되어 집니다.
	 *  
	 *  method body {} : 메서드이름(){메서드바디} 좌에서 보듯이 일반적인 메서드 정의 형식입니다.
	 ...
	 *  
	 *  body{} 내의 return : 만약 메서드가 수행 중간 또는 후에 뭔가를 되돌려줄(return) 의도가
