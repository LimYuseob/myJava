...
public class JTFPass extends JFrame implements ActionListener{
	...
	Container con;
	JTextArea ta = new JTextArea("asdsadasdasd");
	...
	public JTFPass() {
		...
		ta.replaceRange("ABCD", 0, 4);//0~4까지의 문자열을 "ABCD"로 변경합니다.
	}
	void init() {
		...
		//이것도 컨테이너기에 배치가 필요합니다. 해서 생성시 보더레이아웃으로 배치 지정합니다.
		...
		jp1.add("West", lb1);
		//암호필드 값 입력시 입력값을 대체하는 문자 지정..abcd 로 입력하면 지정된
		//문자로 표시됩니다...간단히 메서드로 가능합니다.
		jpf.setEchoChar('&');
		jp1.add("Center",jpf);
		con.add("South",jp1);
		con.add("North",jp);
