 ...
 * 다른 쓰래드는 해당 메서드와 다른 동기화 블럭 또한 접근이 차단 됩니다.(객체에 lock 걸림)
 * 
 * 하지만 일반 메서드는 수행 가능합니다.
 * 
 * 쓰래드가 동기화 메서드를 실행하는 즉시 객체엔 Lock, 수행후엔 Lock 해제, 이때 대기중인
 * 쓰래드는 잘못하면 데드락(Death lock) 상태에 빠질 수 있으며, 이땐 아무것도 하지 못하는 상태가
 * 될 수 있습니다.
 * 
 * 만약 여러분이 메서드 전체에 동기화를 걸지 않고 일부만 적용하고싶을때 동기화 블럭을 사용합니다.
